...
class UserController extends Controller {
	...
	public function register(Request $request) {
		...
		try {
			...
			a:$radUserId = $request->user_id;
			// a:$radUserId = "PP".substr(number_format(time() * rand(), 0, '', ''), 0, '7');
			...
			if($request->Input('ref_user_id') == 'Admin' || $request->Input('ref_user_id') == 'admin'){
				...
				if($totalRefCount > 0){
					$arrStatus = Response::HTTP_NOT_FOUND;
					$arrCode = Response::$statusTexts[$arrStatus];
					$arrMessage = 'Sponsor Id not exist';
					return sendResponse($arrStatus, $arrCode, $arrMessage, '');
				}
			}
			...
			if (empty($getuser)) {
				...
				*/
				...
				$arrCode = Response::$statusTexts[$arrStatus];
				$arrMessage = 'Sponsor already registered exist';
				return sendResponse($arrStatus, $arrCode, $arrMessage, '');
			}
		} catch (Exception $e) {
			dd($e);
	...
	function changePassword(Request $request) {
		try {
			...
			$arrInput['user_id'] = $id;
			$arrRules            = ['otp' => 'required|min:6|max:22'];
		...
		// dd($verify_otp);
			...
			if (!empty($check_useractive)) {
				if(Hash::check($request->input('new_pwd'), $check_useractive->bcrypt_password)){
					...
					return sendResponse($this->statuscode[401]['code'], $arrStatus, $arrMessage, '');
				}
	...
	function changeAddress(Request $request){
		...
		// dd($verify_otp);
			...
			if($request->trn == "" && $request->btc == "" && $request->ltc == "" && $request->eth == "")
			{
				$arrStatus = Response::HTTP_NOT_FOUND;
				$arrCode = Response::$statusTexts[$arrStatus];
			...
			if(!empty($getuser_id)){
				...
				if (!empty($request->Input('sol_add'))) {
					$flag = 3;
					$addData['currency'] = "SOL";
					$addData['currency_address'] = trim($request->Input('sol_add'));
					$addData['ip_address'] = $_SERVER['REMOTE_ADDR'];
					$addBTCStatus = UserWithdrwalSetting::where([['id',$getuser_id],['currency',$addData['currency']],['status',1]])->first();
					// start with 0,1
					if(strlen(trim($request->Input('sol_add'))) >= 26 && strlen(trim($request->Input('sol_add'))) <= 46){
						$split_array = str_split(trim($request->Input('sol_add')));
						if ($split_array[0] == '0') {
						...
						return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Sol address is not valid!', '');
					}
					$new_time = \Carbon\Carbon::now()->addDays(1);
					$token = md5(Auth::user()->user_id.$new_time);
					$addData['block_user_date_time'] = $new_time;
					...
					if(empty($addBTCStatus)){
						$addressStatus = UserWithdrwalSetting::create($addData);
					}else if($addBTCStatus->currency_address != trim($request->Input('sol_add'))){
						$updateAddress['block_user_date_time'] = $new_time;
						$updateAddress['token'] = $token;
						$updateAddress['token_status'] = 0;
						$updateAddress['currency_address'] = trim($request->Input('sol_add'));
						$addressStatus = UserWithdrwalSetting::where('srno', $addBTCStatus->srno)->update($updateAddress);
					}else{
						$flag = 0;
					}
	...
	public function updateUserData(Request $request) {
			...
			if (!empty($request->Input('fullname'))) {
						$rules = array('fullname' => 'required|min:5|max:30|regex:/^[A-Za-z0-9 _]*[A-Za-z0-9][A-Za-z0-9 _]*$/');
					}

                  if (!empty($request->Input('mobile'))) {
						$rules = array('mobile' => 'required|min:8|numeric|digits:10');
					}
...
/*dd($last_entry_in_supermatching->rank);*/
	...
	public function uploadPhotosold(Request $request) {
		 try {
    ...
    $getuser_id = Auth::user()->id;	
		...
		if (!empty($updateData)) {
			$arrStatus = Response::HTTP_OK;
			$arrCode = Response::$statusTexts[$arrStatus];
			$arrMessage = 'Profile Image updated successfully';
			return sendResponse($arrStatus, $arrCode, $arrMessage, '');
		} else {
			$arrStatus = Response::HTTP_OK;
			$arrCode = Response::$statusTexts[$arrStatus];
	...
	public function getcountryFranchiseUsers(Request $request) {
		...
		try {
			...
			$topupReport = User::Leftjoin('tbl_country_new as tc', 'tc.iso_code', '=','tbl_users.country')	
			->where([['tbl_users.is_franchise', '1'],['tbl_users.income_per','=','2'],['tbl_users.status', '=', 'Active'],['tbl_users.id', '!=', $Checkexist]])
			->select('tbl_users.id', 'tbl_users.user_id', 'tbl_users.fullname', 'tc.country')
			->orderBy('tbl_users.entry_time', 'asc');

			$totalRecord = $topupReport->count('tbl_users.id');
	...
	public function getconitentFranchiseUsers(Request $request) {
		try {
			$Checkexist = Auth::User()->id; // check use is active or not
			// ini_set('memory_limit', '-1');
			if (!empty($Checkexist)) {
				...
				$topupReport = User::Leftjoin('tbl_country_new as tc', 'tc.iso_code', '=','tbl_users.country')	
			->where([['tbl_users.is_franchise', '2'],['tbl_users.income_per','=','3'],['tbl_users.status', '=', 'Active'],['tbl_users.id', '!=', $Checkexist]])
			->select('tbl_users.id', 'tbl_users.user_id', 'tbl_users.fullname', 'tc.country')
			->orderBy('tbl_users.entry_time', 'asc');

			$totalRecord = $topupReport->count('id');
			$arrPendings = $topupReport->skip($request->input('start'))->take($request->input('length'))->get();
	...
	public function getglobalFranchiseUsers(Request $request) {
		try {
			$Checkexist = Auth::User()->id; // check use is active or not
			// ini_set('memory_limit', '-1');
			if (!empty($Checkexist)) {
				...
				$topupReport = User::Leftjoin('tbl_country_new as tc', 'tc.iso_code', '=','tbl_users.country')	
			->where([['tbl_users.is_franchise', '3'],['tbl_users.income_per','=','5'],['tbl_users.status', '=', 'Active'],['tbl_users.id', '!=', $Checkexist]])
			->select('tbl_users.id', 'tbl_users.user_id', 'tbl_users.fullname', 'tc.country')
			->orderBy('tbl_users.entry_time', 'asc');
