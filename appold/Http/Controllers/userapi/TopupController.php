...
class TopupController extends Controller {
			...
			// check invoice already exist
				...
				if (!empty($plan)) {
					...
					{	
						...
						//$getlevel = $this->    public function pay_directbulk($user_id, $amount, $direct_income, $invoice_id) {
							//($usersid, $invoice[0]->price_in_usd, $direct_income, $invoice_id);
					}
	...
	public function getTopup(Request $request) {
		...
		$transcation_type = $request->transcation_type;
			...
			$users = User::select('tbl_users.id','tbl_users.user_id','tbl_users.virtual_parent_id','tbl_users.ref_user_id','tbl_dashboard.total_investment','tbl_dashboard.active_investment','tbl_users.position')->join('tbl_dashboard', 'tbl_dashboard.id', '=', 'tbl_users.id')->where([['tbl_users.user_id', '=', Auth::user()->user_id], ['tbl_users.status', '=', 'Active']])->first();
			if(empty($users)){
			...
			if(!empty($check_user_hits)){
						$arrStatus = Response::HTTP_OK;
							$arrCode = Response::$statusTexts[$arrStatus];
							$arrMessage = 'Amount Added successfully';
							return sendResponse($arrStatus, $arrCode, $arrMessage, '');
		...
		try {			
         ...
         /* to check user downline or not */
			...
			if (!empty($getPrice)) {
				...
				// dd($getPrice->entry_time, $now, $this->today, $testdate);
				if ($testdate <= 2) {
			...
			if (!empty($users)) {
				...
				if (!empty($packageExist)) {
					...
					$comp_lock_per = $packageExist->compounding_lock_percentage;
					$total_comp_lock_per = $packageExist->total_compounding_lock_percentage;
					...
					if($transcation_type == 2){
						...
						/*$balance = Dashboard::where('id',$userId)->selectRaw('round(fund_wallet - fund_wallet_withdraw,2) as fundbalance,round(top_up_wallet - top_up_wallet_withdraw,2) as purchasebalance')->first(); 
					...
					if ($Productcost >= $packageExist->min_hash && $Productcost <= $packageExist->max_hash) {
						...
						if ($Productcost >= 35000) {
							$amount = $Productcost;

	// ======Add franchise=========

							if($amount >=35000 && $amount <75000){

								$income_per=2;

							  if($users->is_franchise !=1 && $users->income_per !=2){

								   $updateUser = User::where('id', $users->id)->update(['is_franchise' => '1','income_per'=>$income_per]);
								}
							}
							...
							{
							   $income_per=3;
							   $updateUser = User::where('id', $users->id)->update(['is_franchise' => '2','income_per'=>$income_per]);
							}

							if($amount >= 150000)
							{
							   $income_per=5;
							   $updateUser = User::where('id', $users->id)->update(['is_franchise' => '3','income_per'=>$income_per]);
							}
						...
						$Topupdata['compound_amount'] = $amount;
						$Topupdata['compounding_lock_percentage'] = $comp_lock_per;
						$Topupdata['total_compound_amount'] = ($amount*$total_comp_lock_per)/100;
						$Topupdata['total_compounding_lock_percentage'] = $total_comp_lock_per;
						...
						$Topupdata['direct_income'] = $direct_income;
						...
						if (!empty($storeId)) {
							...
							if (!empty($plan)) {
								...
								if ($binary_plan == '1') {
									...
									if ($checktopup <= 0) {
										$updateLCountArrDirectBusiness['paid_directs'] = DB::raw('paid_directs + 1');
										$updateRCountArrDirectBusiness['paid_directs'] = DB::raw('paid_directs + 1');
									}
								 ...
								 /*$ref_direct_income = Topup::where('user_status','Active')->where('id',$users->ref_user_id)->orderBy('srno', 'desc')->select('direct_income')->first();

							if(!empty($ref_direct_income))
            				{	*/
            					/*dd($direct_plan , $direct_income);*/
								if ($direct_plan == '1' && $direct_income > 0) { //dd('ref_direct_income');
									  // check rank for direct user to give direct incomes
