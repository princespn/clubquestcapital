      ...
      //  dd("hi");
        try{
           ...
           // $percentagedata = Rank::where([['rank', '=', $rank]])->pluck('income_percentage')->first();
            ...
            $direct_income = Topup::where('user_status','Active')->where('id',$ref_user_id)->orderBy('srno', 'desc')->select('direct_income')->first();
           
            $percentage = (!empty($direct_income))?$direct_income->direct_income:10;
            ...
            //// $ref_count = User::select('id')->where('topup_status','0')->where('id',$ref_user_id)->count('id');

            $ref_count = User::where('id',$ref_user_id)->where('type','')->select('topup_status')->first();
          
            //dd($ref_count->topup_status);
           if(!empty($ref_count))
            {
                ...
                $check_if_ref_exist = Topup::where('id',$ref_user_id)->orderBy('srno', 'desc')->select('roi_status')->first();

                if ($ref_count->topup_status === "0") { //dd('qqssq');
                    $laps = $working_wallet_amount;
                    $working_wallet_amount = 0;
                    $remark = "Not having topup";
                }
                elseif($check_if_ref_exist->roi_status === "Inactive") {
                    $laps = $working_wallet_amount;
                    $working_wallet_amount = 0;
