...
class DashboardController extends Controller
{
    ...
    public function getDashboardSummary(Request $request)
    {
        ...
        if (isset($request->frm_date) && isset($request->to_date)) {
            ...
            $AwardWinner = $AwardWinner->sum('tal.award');
             ...
             //Total Reward Income balance

            $RewardWinner = DB::table('tbl_reward_achievers')
            ->join('tbl_reward_setting as trs', 'trs.id', '=', 'tbl_reward_achievers.reward_id');

            if (isset($request->id) && !empty($request->id)) {
                $RewardWinner = $RewardWinner->where('user_id', $request->id);
            }
            $totalrewardwall = $RewardWinner->sum('trs.reward_amount');

             //Total Reward Income balance End

            //Total Team Associative Reward Income balance

            $TeamRewardWinner = DB::table('tbl_team_direct_reward_achievers')
            ->join('tbl_direct_reward_setting as tdrs', 'tdrs.id', '=','tbl_team_direct_reward_achievers.direct_reward_id');
            if (isset($request->id) && !empty($request->id)) {
                $TeamRewardWinner = $TeamRewardWinner->where('user_id', $request->id);
            }
            $totalteamrewardball = $TeamRewardWinner->sum('tdrs.reward_amount');

             //Total Team Associative Reward Income balance End
            ...
            //Total Franchise Income balance
             $franchiseincome = DB::table('tbl_franchise_income')
            ->join('tbl_users as tu', 'tu.id', '=', 'tbl_franchise_income.to_user_id');
            if (isset($request->id) && !empty($request->id)) {
                $franchiseincome = $franchiseincome->where('to_user_id', $request->id);
            }
            $totalfranchisewall = $franchiseincome->sum('tbl_franchise_income.amount');
            // $totalfranchisewall = Dashboard::sum('franchise_income');

            //Total Compounding Income balance
             $totalcompwall = Dashboard::sum('Compounding_wallet');
            $totalcompWithdrawwal = Dashboard::sum('Compounding_wallet_withdraw');
            $totalcompoundingwall = $totalcompwall - $totalcompWithdrawwal;
       ...
       $supermatching_income=SupperMatchingIncome::sum('amount'); 
        ...
        $arrSummaryData['total_reward_income']        = number_format($totalrewardwall, 2, '.', '');
        $arrSummaryData['total_team_reward_income']        = number_format($totalteamrewardball, 2, '.', '');

        $arrSummaryData['total_income']             = number_format($directIncomeCount + $binaryIncomeCount + $totalfranchisewall + $roiIncomeCount + $totalrewardwall + $totalteamrewardball + $totalcompoundingwall, 2, '.', '');

        $arrSummaryData['total_franchise_income']        = number_format($totalfranchisewall, 2, '.', '');
        $arrSummaryData['total_compounding_income']        = number_format($totalcompoundingwall, 2, '.', '');
     ...
     public function getAccountWallet(Request $request)
    {
        ...
        if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu.user_id','tu.fullname','tbl_dashboard.fund_wallet','tbl_dashboard.fund_wallet_withdraw','tbl_dashboard.working_wallet','tbl_dashboard.working_wallet_withdraw','tbl_dashboard.entry_time');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
