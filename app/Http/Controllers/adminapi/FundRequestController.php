...
class FundRequestController extends Controller
{
    ...
    public function rejectFundRequest(Request $request) {
      ...
      public function fundRequest1(Request $request) {
        $rules = array(
            'user_id' => 'required',
            'amount' => 'required',
        );
    ...
    public function WalletTransactionReport(Request $request) {
        $arrInput = $request->all();
        $query = WalletTransactionLog::select(DB::raw('COALESCE(tu.user_id," ") as user_id'),'tu1.user_id as from_user_id',DB::raw('COALESCE(tu.fullname," ") as fullname'),'tu1.fullname as from_fullname','tbl_wallet_transaction_log.amount','tbl_wallet_transaction_log.remark','tbl_wallet_transaction_log.entry_time','tbl_wallet_transaction_log.transaction_type',DB::raw('CASE WHEN tbl_wallet_transaction_log.wallet_type = 1 THEN "Working Wallet" WHEN tbl_wallet_transaction_log.wallet_type = 2 THEN "Purchase Wallet" WHEN tbl_wallet_transaction_log.wallet_type = 3 THEN "Fund Wallet" WHEN tbl_wallet_transaction_log.wallet_type = 4 THEN "Development Bonus" ELSE "" END as wallet_type'),'tbl_wallet_transaction_log.ip_address')
        ...
        if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
            $qry = $query;
            $qry = $qry->select(DB::raw('COALESCE(tu.user_id," ") as user_id'),'tu1.user_id as from_user_id',DB::raw('COALESCE(tu.fullname," ") as fullname'),'tu1.fullname as from_fullname','tbl_wallet_transaction_log.amount','tbl_wallet_transaction_log.remark','tbl_wallet_transaction_log.entry_time','tbl_wallet_transaction_log.transaction_type',DB::raw('CASE WHEN tbl_wallet_transaction_log.wallet_type = 1 THEN "Working Wallet" WHEN tbl_wallet_transaction_log.wallet_type = 2 THEN "Purchase Wallet" WHEN tbl_wallet_transaction_log.wallet_type = 3 THEN "New Fund Wallet" ELSE "" END as wallet_type'),'tbl_wallet_transaction_log.ip_address');
            $records = $qry->get();
            $res = $records->toArray();
            if (count($res) <= 0) {
                return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
