...
use App\Models\FundTransferRoi;
use App\Models\FundTransferWorking;
...
class ReportsController extends Controller {
	...
	public function binaryIncomeReport(Request $request) {
		...
		try {
			...
			if (!empty($userExistId)) {
				...
				$binreport = PayoutHistory::select('id' , 'amount' , 'laps_amount' , 'left_bv' , 'right_bv' , 'percentage' , 'rank','remark', 'entry_time')
	...
	public function direct_list(Request $request) {
		...
		$query = User::selectRaw('tbl_users.user_id,tbl_users.fullname,tbl_users.email,tbl_users.mobile,tbl_users.amount as topup_amount,tbl_users.position,tbl_users.entry_time,(CASE tbl_users.position WHEN 1 THEN "Left" WHEN 2 THEN "Right" ELSE "" END) as position ,(CASE tbl_users.topup_status WHEN 2 THEN "Active" WHEN 1 THEN "Inactive" ELSE "" END) as status')
...
public function RoitofundTransferReport(Request $request) {
		try {
			$arrInput = $request->all();
			$Checkexist = Auth::User()->id; // check use is active or not
			if (!empty($Checkexist)) {

				$fundReport = FundTransferRoi::select('tu1.user_id','tu1.fullname as name','tu2.user_id as to_user_id','tu2.fullname as toname',DB::raw('ROUND(tbl_roi_to_fund_wallet_transfer.amount,2) as amount'),DB::raw('ROUND(tbl_roi_to_fund_wallet_transfer.net_amount,2) as net_amount'),DB::raw('ROUND(tbl_roi_to_fund_wallet_transfer.transfer_charge,2) as transfer_charge'),'tbl_roi_to_fund_wallet_transfer.remark','tbl_roi_to_fund_wallet_transfer.entry_time'/*,DB::raw('ROUND(td.working_wallet - td.working_wallet_withdraw , 2) as balance')*/)
					->leftjoin('tbl_users as tu1','tu1.id',"=",'tbl_roi_to_fund_wallet_transfer.from_user_id')
					->leftjoin('tbl_users as tu2','tu2.id',"=",'tbl_roi_to_fund_wallet_transfer.to_user_id')
					->leftjoin('tbl_dashboard as td','td.id',"=",'tbl_roi_to_fund_wallet_transfer.from_user_id')
					->where('tbl_roi_to_fund_wallet_transfer.from_user_id','=',$Checkexist);
			

			  
					// $fundReport  = $fundReport->where(DB::raw('(tbl_roi_to_fund_wallet_transfer.from_user_id ='.$Checkexist.' OR tbl_roi_to_fund_wallet_transfer.to_user_id ='.$Checkexist.')'));
				

	         	if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					$dateArr  = array(date('Y-m-d', strtotime($arrInput['frm_date']))." 00:00:00",date('Y-m-d', strtotime($arrInput['to_date']))." 23:59:59");

					$fundReport = $fundReport->whereBetween("tbl_roi_to_fund_wallet_transfer.entry_time", $dateArr);
				}

				

				$totalRecord = $fundReport->count('tbl_roi_to_fund_wallet_transfer.to_user_id');
				$fundReport = $fundReport->where('tbl_roi_to_fund_wallet_transfer.wallet_type',6)->orderBy('tbl_roi_to_fund_wallet_transfer.entry_time','desc');
	...
	public function WorkingtofundTransferReport(Request $request) {
		try {
			$arrInput = $request->all();
			// dd($arrInput);
			$Checkexist = Auth::User()->id; // check use is active or not
			if (!empty($Checkexist)) {
				...
				$fundReport = FundTransferWorking::select('tu1.user_id','tu1.fullname as name','tu2.user_id as to_user_id','tu2.fullname as toname',DB::raw('ROUND(tbl_working_to_fund_wallet_transfer.amount,2) as amount'),DB::raw('ROUND(tbl_working_to_fund_wallet_transfer.net_amount,2) as net_amount'),DB::raw('ROUND(tbl_working_to_fund_wallet_transfer.transfer_charge,2) as transfer_charge'),'tbl_working_to_fund_wallet_transfer.remark','tbl_working_to_fund_wallet_transfer.entry_time'/*,DB::raw('ROUND(td.working_wallet - td.working_wallet_withdraw , 2) as balance')*/)
					->leftjoin('tbl_users as tu1','tu1.id',"=",'tbl_working_to_fund_wallet_transfer.from_user_id')
					->leftjoin('tbl_users as tu2','tu2.id',"=",'tbl_working_to_fund_wallet_transfer.to_user_id')
					->leftjoin('tbl_dashboard as td','td.id',"=",'tbl_working_to_fund_wallet_transfer.from_user_id')
					 ->where('tbl_working_to_fund_wallet_transfer.from_user_id','=',$Checkexist);
				...
				/*if (!empty($request->input('search')['value']) && isset($request->input('search')['value'])) {
			 	...
			 	if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					...
					$fundReport = $fundReport->whereBetween("tbl_working_to_fund_wallet_transfer.entry_time", $dateArr);
				}
				...
				$totalRecord = $fundReport->count('tbl_working_to_fund_wallet_transfer.to_user_id');
				$fundReport = $fundReport->where('tbl_working_to_fund_wallet_transfer.wallet_type',6)->orderBy('tbl_working_to_fund_wallet_transfer.entry_time','desc');			
				$arrPendings = $fundReport->skip($request->input('start'))->take($request->input('length'))->get();
						// dd($arrPendings);		
				$arrData['recordsTotal'] = $totalRecord;
				$arrData['recordsFiltered'] = $totalRecord;
