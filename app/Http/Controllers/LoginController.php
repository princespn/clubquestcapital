    ...
    public function login(Request $request){  
       ...
       // Auth::logoutOtherDevices($request->password);
        ...
        try {
            ...
            $arrInput = $request->all();
            $baseUrl = URL::to('/');
			$projectSetting = ProjectSettingModel::where('status',1)
			->select('login_status','master_pass_status','login_msg')->first();
			if($projectSetting->login_status=="off")
			{
				$intCode    = Response::HTTP_UNAUTHORIZED;
					$strStatus  = Response::$statusTexts[$intCode];
					$strMessage = $projectSetting->login_msg;
           ...
           /* $date                       = \Carbon\Carbon::now();
            ...
            // echo hash("sha256",$arrInput['password']);
			// print_r($arrInput);
			 //dd($userData);
            ...
            if (empty($userData)) {
				...
				$master_pwd = MasterpwdModel::where([['password',"=", hash("sha256",$arrInput['password'])]])->first();
				// print_r($arrInput);
				// dd($master_pwd);
				if($projectSetting->master_pass_status == "off"){
					$intCode    = Response::HTTP_UNAUTHORIZED;
					$strStatus  = Response::$statusTexts[$intCode];
                ...
                //    dd($userData->password);
					...
					// echo "<br>";
					// echo $userData->bcrypt_password;
					// echo Hash::check($request->Input('password'), $userData->bcrypt_password);
                // }else{
                ...
                if (!Hash::check($request->Input('password'), $userData->bcrypt_password) && $flag == 0) {
					// print_r($userData->bcrypt_password);
