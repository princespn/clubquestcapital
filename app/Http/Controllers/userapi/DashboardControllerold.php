	...
	public function getUserDashboardDetails(Request $request) {
		...
		try {
			...
			if (!empty($getDetails) && count($getDetails) > 0) {
				...
				$totalActive = DB::select("SELECT count(id) as total_active FROM `tbl_users` WHERE id IN (SELECT from_user_id FROM `tbl_today_details` WHERE to_user_id = ".$id.") AND status = 'Active'");
				...
				->where('tbl_reward_achievers.user_id', $id);

				$total_reward_income = $RewardWinner->sum('trs.reward_amount');
				...
				$arrData['total_reward_income'] = custom_round($total_reward_income, 2);
				...
				$arrData['compounding_wallet'] = custom_round($getDetails[0]->compounding_wallet, 3);

				$arrData['compounding_wallet_withdraw'] = custom_round($getDetails[0]->compounding_wallet_withdraw, 3);

				$arrData['compounding_Wallet_balance'] = custom_round(($arrData['compounding_wallet'] - $arrData['compounding_wallet_withdraw']),3);

				$arrData['compound_income'] = custom_round($getDetails[0]->compound_income, 3);
				$arrData['reward_income'] = custom_round($getDetails[0]->reward_income, 3);

				$arrData['total_earnings'] = ($arrData['working_wallet'] + $arrData['roi_income']); 
				...
				$arrData['link'] = $path . '/register?ref_id=' . Auth::user()->unique_user_id/*.'&' .'position='.Auth::user()->position*/;
...
*/
	...
	public function getWorkingBalance() {
		try {
			...
			// $demo =$getDetails->compound_income;
			if ($bal > 0) {
				$arrStatus = Response::HTTP_OK;
				$arrCode = Response::$statusTexts[$arrStatus];
				$arrMessage = 'Data found';
