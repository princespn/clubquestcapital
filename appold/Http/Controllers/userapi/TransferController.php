...
class TransferController extends Controller
{
    ...
    public function UserToUserTransfer(Request $request)
    {
        ...
        try {
            ...
            $to_user_dashboard = Dashboard::select('fund_wallet','fund_wallet_withdraw')->where('id',$transfer_user_id)->first();

            $to_user_topup_bal = $to_user_dashboard->fund_wallet - $to_user_dashboard->fund_wallet_withdraw;
            

            if (!empty($users)) {
                if ($users->user_id != $request->Input('to_user_id')) {
                    $trans_bal = $users->fund_wallet - $users->fund_wallet_withdraw;
                    ...
                    if ($request->input('amount') > 0) {
                        if ($trans_bal >= $request->input('amount')) {
                            ...
                            if (!empty($to_user_id)) {
                                ...
                                $tempData = [];
                                //---------update level income wd---------//
                                $updateCoinData['fund_wallet_withdraw'] = round(($users->fund_wallet_withdraw + $request->input('amount')), 7);
                                $updateCoinData['total_withdraw'] = round(($users->fund_wallet_withdraw + $request->input('amount')), 7);
                                $updateCoinData['usd'] = round(($users->usd - $request->input('amount')), 7);
                                $updateCoinData = Dashboard::where('id', $users->id)->limit(1)->update($updateCoinData);

                                $to_userid_transfer_details = Dashboard::where([['id', '=', $to_user_id],])->select('fund_wallet', 'total_profit', 'usd')->first();

                                $to_userid_transfer_wallet = $to_userid_transfer_details->fund_wallet;
                                ...
                                $updateTouserData['fund_wallet'] = $to_userid_transfer_wallet + $request->input('amount');
                                ...
                                $Trandata['prev_balance'] = $users->fund_wallet - $users->fund_wallet_withdraw; 
                                $Trandata['final_balance'] = $Trandata['prev_balance'] - $request->input('amount');
                        ...
                        $arrStatus   = Response::HTTP_NOT_FOUND;
                        $arrCode     = Response::$statusTexts[$arrStatus];
                        $arrMessage  = 'Fund wallet balance should be greater than 0';
                        return sendResponse($arrStatus, $arrCode, $arrMessage, '');
                    }
                } else {
                    $arrStatus   = Response::HTTP_NOT_FOUND;
    ...
    public function RoiToPurchaseSelfTransfer(Request $request)
        {
        ...
        try {
            ...
            if (!empty($id)) {
                ...
                $working_bal = Dashboard::selectRaw('roi_income - roi_income_withdraw as balance')->where('id',$id)->first();
                $amount =$request->amount;
                if($working_bal->balance >= $amount ){

                    $updateCoinData = array();

                    $updateCoinData['roi_income_withdraw'] = DB::raw('roi_income_withdraw +'.$amount.'');
                    $updateCoinData['fund_wallet'] = DB::raw('fund_wallet +'.$amount.'');
                    $updateqryto = Dashboard::where('id', $id)->limit(1)->update($updateCoinData);
                    ...
                    $fundData['from_user_id'] = $id;
                    $fundData['amount'] = $amount;
                    $fundData['net_amount'] = $amount;
                    $fundData['status'] = 1;
                    $fundData['from_wallet_type'] = 3;
                    // $fundData['transfer_charge'] = $per;
                    $fundData['to_wallet_type'] = 8;
                    $fundData['entry_time'] = $this->today;
    ...
    public function CompoundToPurchaseSelfTransfer(Request $request)
        {
        ...
        try {
            ...
            if (!empty($id)) {
                ...
                $working_bal = Dashboard::selectRaw('compounding_wallet - compounding_wallet_withdraw as balance')->where('id',$id)->first();
                $amount =$request->amount;
                if($working_bal->balance >= $amount ){

                    $updateCoinData = array();

                    $updateCoinData['compounding_wallet_withdraw'] = DB::raw('compounding_wallet_withdraw +'.$amount.'');
                    $updateCoinData['fund_wallet'] = DB::raw('fund_wallet +'.$amount.'');
                    $updateqryto = Dashboard::where('id', $id)->limit(1)->update($updateCoinData);
