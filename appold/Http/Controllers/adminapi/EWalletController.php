...
class EWalletController extends Controller {
	...
	public function getWithdrwalVerify(Request $request) {
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu.user_id','tu.fullname','cn.country',DB::raw('tbl_withdrwal_pending.amount+tbl_withdrwal_pending.deduction as amount'),'tbl_withdrwal_pending.deduction',DB::raw('tbl_withdrwal_pending.amount as net_amount'),'tbl_withdrwal_pending.network_type as currency_type','tbl_withdrwal_pending.to_address as address',DB::raw('(CASE tbl_withdrwal_pending.status WHEN 0 THEN "Unpaid" WHEN 1 THEN "Paid" END) as status'),DB::raw('(CASE tbl_withdrwal_pending.withdraw_type WHEN 2 THEN "Working" WHEN 3 THEN "ROI" WHEN 7 THEN "Passive Income" END) as wallet_type'),'tbl_withdrwal_pending.entry_time as Date','tbl_withdrwal_pending.ip_address','tbl_withdrwal_pending.topupfrom');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
	...
	public function getWithdrwalConfirmed(Request $request) {
		...
		->leftjoin('tbl_country_new as cn','tu.country','=','cn.iso_code')
			->select('tbl_withdrwal_confirmed.sr_no','tbl_withdrwal_confirmed.transaction_hash',
				...
				DB::raw('(CASE tbl_withdrwal_confirmed.withdraw_type WHEN 2 THEN "Working" WHEN 3 THEN "Roi" WHEN 4 THEN "Self Working" WHEN  5 THEN "Self Roi" WHEN  7 THEN "Passive Income" ELSE "Working" END) as withdraw_type'),'cn.country','tu.perfect_money_address');
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu.user_id', 'tu.fullname','tbl_withdrwal_confirmed.amount','tbl_withdrwal_confirmed.deduction',DB::raw('(tbl_withdrwal_confirmed.amount + tbl_withdrwal_confirmed.deduction) as total_amount' ),'tbl_withdrwal_confirmed.network_type', 'tbl_withdrwal_confirmed.to_address',DB::raw('(CASE tbl_withdrwal_confirmed.withdraw_type WHEN 2 THEN "Working" WHEN 3 THEN "Roi" WHEN 4 THEN "Self Working" WHEN  5 THEN "Self Roi" WHEN  7 THEN "Passive Income" ELSE "" END) as withdraw_type'), DB::raw('(CASE tbl_withdrwal_confirmed.status WHEN 0 THEN "Unpaid" WHEN 1 THEN "Paid" END) as status') , 'cn.country', 'tbl_withdrwal_confirmed.entry_time');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
...
public function getWithdrwalVerified(Request $request) {
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu.user_id', 'tu.fullname','tbl_withdrwal_pending.amount','tbl_withdrwal_pending.deduction','tbl_withdrwal_pending.network_type', 'tbl_withdrwal_pending.to_address',DB::raw('(CASE tbl_withdrwal_pending.withdraw_type WHEN 2 THEN "Working" WHEN 3 THEN "Roi" WHEN 4 THEN "Self Working" WHEN  5 THEN "Self Roi" WHEN  7 THEN "Passive Income" ELSE "" END) as withdraw_type'), DB::raw('(CASE tbl_withdrwal_pending.status WHEN 0 THEN "Unpaid" WHEN 1 THEN "Paid" END) as status') ,'tbl_withdrwal_pending.entry_time');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
...
/**
  ...
  //           }
		...
		$arrInput['user_id'] = $id;
			...
			$verify_otp = Otp::select('otp_id','otp_status','otp')
		    ->where('id', Auth::user()->id)
			->where('otp', md5($request->otp))
			->where('otp_status', '=',0)
		    ->orderBy('entry_time', 'desc')->first();
            if (!empty($verify_otp)) {
            	Otp::where('otp_id', $verify_otp->otp_id)->delete();
            	$verify_otp['status'] = 200;
                if ($verify_otp['status'] == 200) {
                } else {
                    $arrStatus = Response::HTTP_NOT_FOUND;;
                    $arrCode = Response::$statusTexts[$arrStatus];
...
*/
	...
	public function getDirectIncome(Request $request) {
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu1.user_id as to_user_id'  ,'tu.user_id as from_user_id' , 'tbl_directincome.amount' ,'tbl_directincome.laps_amount' , 'tbl_directincome.remark' ,'tbl_directincome.status', 'tbl_directincome.entry_time');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
	...
	public function allFranchiseUser(Request $request) {
		$arrInput = $request->all();

		$query = DB::table('tbl_users')
					->select('user_id','mobile','fullname',DB::raw('(CASE is_franchise WHEN 1 THEN "COUNTRY
						AGENT" WHEN 2 THEN "CONTINENT
						FRANCHISE" WHEN 3 THEN "GLOBAL
						PARTNER" END) as is_franchise'),'email')
		...
		// if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
		// 	$qry = $query;
		// 	$qry = $qry->select('tu.user_id','tu.mobile', 'tu.fullname','tu2.user_id as sponser_user_id', 'tu2.fullname as sponser_fullname',DB::raw('tu.power_r_bv + tu.power_l_bv as total_direct'), 'tbl_qualified_user_list.entry_time');
		// 	$records = $qry->get();
		// 	$res = $records->toArray();
		// 	if (count($res) <= 0) {
		// 		return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
	...
	public function getFranchiseIncome(Request $request) {
		...
		$query = FranchiseIncome::join('tbl_users as tu', 'tu.id', '=', 'tbl_franchise_income.from_user_id')
			...
			->select('tbl_franchise_income.entry_time','tbl_franchise_income.pin','tbl_franchise_income.amount','tbl_franchise_income.percentage','tbl_franchise_income.on_amount', 'tbl_franchise_income.pin', 'tu.user_id as from_user_id', 'tu1.user_id as to_user_id', 'tu.fullname as from_fullname', 'tu1.fullname as to_fullname');
		
		if (isset($arrInput['user_id'])) {
			$query = $query->where('tu.user_id', $arrInput['user_id'])->orWhere('tu1.user_id', $arrInput['user_id']);
		}
		// if (isset($arrInput['user_id'])) {
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tbl_franchise_income.entry_time','tbl_franchise_income.pin','tbl_franchise_income.amount','tbl_franchise_income.percentage','tbl_franchise_income.on_amount', 'tbl_franchise_income.pin', 'tu.user_id as from_user_id', 'tu1.user_id as to_user_id', 'tu.fullname as from_fullname', 'tu1.fullname as to_fullname');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
	...
	public function getRewardIncomeReport(Request $request) {
		try {
			...
			if (!empty($Checkexist)) {
				...
				$query = AllRewardsAchievers::join('tbl_users as tu', 'tu.id', '=', 'tbl_reward_achievers.user_id')
				->join('tbl_reward_setting as trs', 'trs.id', '=', 'tbl_reward_achievers.reward_id')
									   ->select('tbl_reward_achievers.reward_name' ,'tbl_reward_achievers.status','trs.reward_amount','tbl_reward_achievers.entry_time','tu.user_id as user_id','tu.fullname as fullname');
				...
				if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					$arrInput['frm_date'] = date('Y-m-d', strtotime($arrInput['frm_date']));
					$arrInput['to_date'] = date('Y-m-d', strtotime($arrInput['to_date']));
					$query = $query->whereBetween(DB::raw("DATE_FORMAT(tbl_reward_achievers.entry_time,'%Y-%m-%d')"), [$arrInput['frm_date'], $arrInput['to_date']]);
				}
				// if (!empty($arrInput['search']['value']) && isset($arrInput['search']['value'])) {
				...
				if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tbl_reward_achievers.reward_name' ,'tbl_reward_achievers.status','trs.reward_amount','tbl_reward_achievers.entry_time','tu.user_id as user_id','tu.fullname as fullname');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
			...
			return sendresponse($this->statuscode[200]['code'], $this->statuscode[200]['status'], 'Records found', array('data'=>$var));
				...
				$query = $query->orderBy('tbl_reward_achievers.entry_time', 'desc');
				$totalRecord = $query->count('tbl_reward_achievers.id');
	...
	public function getTeamRewardIncomeReport(Request $request) {
		try {
			$Checkexist = Auth::User()->id;
			...
			if (!empty($Checkexist)) {
				...
				$query = TeamRewardsAchievers::join('tbl_users as tu', 'tu.id', '=', 'tbl_team_direct_reward_achievers.user_id')
				->join('tbl_direct_reward_setting as tdrs', 'tdrs.id', '=','tbl_team_direct_reward_achievers.direct_reward_id')
									   ->select('tdrs.required_direct','tdrs.reward_amount' ,'tbl_team_direct_reward_achievers.status','tbl_team_direct_reward_achievers.entry_time','tu.user_id as user_id','tu.fullname');
...
// dd($query);
				...
				if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					$arrInput['frm_date'] = date('Y-m-d', strtotime($arrInput['frm_date']));
					$arrInput['to_date'] = date('Y-m-d', strtotime($arrInput['to_date']));
					$query = $query->whereBetween(DB::raw("DATE_FORMAT(tbl_team_direct_reward_achievers.entry_time,'%Y-%m-%d')"), [$arrInput['frm_date'], $arrInput['to_date']]);
				}

				if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tdrs.required_direct','tdrs.reward_amount' ,'tbl_team_direct_reward_achievers.status','tbl_team_direct_reward_achievers.entry_time','tu.user_id as user_id','tu.fullname');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
			...
			return sendresponse($this->statuscode[200]['code'], $this->statuscode[200]['status'], 'Records found', array('data'=>$var));
				...
				$query = $query->orderBy('tbl_team_direct_reward_achievers.entry_time', 'desc');
				$totalRecord = $query->count('tbl_team_direct_reward_achievers.id');
				// $totalRecord = $query->get()->count();
				$arrDirectInc = $query->skip($arrInput['start'])->take($arrInput['length'])->get();
				// dd($arrDirectInc);
				$arrData['recordsTotal'] = $totalRecord;
	...
	public function getCompoundingIncomeReport(Request $request) {
		...
		try {
			...
			if (!empty($userExistId)) {
				...
				$ComoundingDailyBonus = CompoundDailyBouns::join('tbl_users as tu', 'tu.id','=', 'tbl_compound_dailybouns.id')
								// ->join('tbl_topup as tt', 'tt.pin', '=', 'tbl_compound_dailybouns.pin')
								->select('tbl_compound_dailybouns.pin','tbl_compound_dailybouns.amount','tbl_compound_dailybouns.on_amount','tu.user_id as user_id','tu.fullname as fullname','tbl_compound_dailybouns.entry_time','tbl_compound_dailybouns.status');
								

				// $ComoundingDailyBonus = DailyBonus::select('tbl_compound_dailybouns.amount' , 'tbl_topup.pin' , 'tbl_compound_dailybouns.entry_time' ,'tbl_compound_dailybouns.status' , 'tbl_topup.amount as name')
				// 						->join('tbl_topup' , 'tbl_compound_dailybouns.pin' , '=' , 'tbl_topup.pin')
				// 						->where('tbl_compound_dailybouns.id' , '=' , $userExistId->id);
				
				// dd($ComoundingDailyBonus);

				if (isset($arrInput['user_id'])) {
				    $ComoundingDailyBonus  = $ComoundingDailyBonus->where('tu.user_id', $arrInput['user_id']);
				}

				if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					$ComoundingDailyBonus = $ComoundingDailyBonus->whereBetween(DB::raw("DATE_FORMAT(tbl_compound_dailybouns.entry_time,'%Y-%m-%d')"), [date('Y-m-d', strtotime($arrInput['frm_date'])), date('Y-m-d', strtotime($arrInput['to_date']))]);
				}

				/*if (!empty($request->input('search')['value']) && isset($request->input('search')['value'])) {
					
					$fields = getTableColumns('tbl_compound_dailybouns');
					$search = $request->input('search')['value'];
					$ComoundingDailyBonus = $ComoundingDailyBonus->where(function ($ComoundingDailyBonus) use ($fields, $search) {
						foreach ($fields as $field) {
							$ComoundingDailyBonus->orWhere('tbl_compound_dailybouns.' . $field, 'LIKE', '%' . $search . '%');
						}
						$ComoundingDailyBonus->orWhere('tp.name', 'LIKE', '%' . $search . '%');
					});
				}*/


				if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $ComoundingDailyBonus;
			$qry = $qry->select('tbl_compound_dailybouns.pin','tbl_compound_dailybouns.amount','tbl_compound_dailybouns.on_amount','tu.user_id as user_id','tu.fullname as fullname','tbl_compound_dailybouns.entry_time','tbl_compound_dailybouns.status');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
			...
			return sendresponse($this->statuscode[200]['code'], $this->statuscode[200]['status'], 'Records found', array('data'=>$var));
				...
				$totalRecord = $ComoundingDailyBonus->count('tbl_compound_dailybouns.id');
				$ComoundingDailyBonus = $ComoundingDailyBonus->orderBy('tbl_compound_dailybouns.entry_time', 'desc');
				// $totalRecord = $ComoundingDailyBonus->count();
				$arrPendings = $ComoundingDailyBonus->skip($request->input('start'))->take($request->input('length'))->get();
				// dd($arrPendings);
				$arrData['recordsTotal'] = $totalRecord;
				$arrData['recordsFiltered'] = $totalRecord;
				$arrData['records'] = $arrPendings;
	...
	public function getBinaryIncome(Request $request) {
		...
		$query = BinaryIncome::join('tbl_users as tu', 'tu.id', '=', 'tbl_payout_history.user_id')
							  ->select('tbl_payout_history.rank','tbl_payout_history.percentage','tbl_payout_history.entry_time','tbl_payout_history.laps_bv','tbl_payout_history.laps_amount','tbl_payout_history.left_bv','tbl_payout_history.right_bv','tbl_payout_history.amount', 'tu.user_id', 'tu.fullname','tbl_payout_history.remark');
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu.user_id','tbl_payout_history.amount','tbl_payout_history.left_bv','tbl_payout_history.right_bv','tbl_payout_history.laps_amount','tbl_payout_history.percentage','tbl_payout_history.entry_time');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
...
*/
  ...
  //           }
    ...
    public function rejectedWithdrawalReport(Request $request) {
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->select('tu.user_id', 'tu.fullname','tbl_withdrwal_pending.amount','tbl_withdrwal_pending.deduction',DB::raw('(tbl_withdrwal_pending.amount + tbl_withdrwal_pending.deduction) as total_amount' ),'tbl_withdrwal_pending.network_type', 'tbl_withdrwal_pending.to_address', DB::raw('(CASE tbl_withdrwal_pending.status WHEN 0 THEN "Unpaid" WHEN 1 THEN "Paid" WHEN 2 THEN "Rejected" END) as status') ,'tbl_withdrwal_pending.entry_time');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
