        ...
        try {
           ...
           // }
            ...
            if (empty($currency_address)) {
                ...
                return sendResponse($this->statuscode[401]['code'], $this->statuscode[401]['status'],'Please update '.$request->Currency_type.' address to withdraw amount','');
            ...
            $dash = Dashboard::where('id', $id)->select('roi_income','roi_income_withdraw')->first();
            $diff = $dash->roi_income - $dash->roi_income_withdraw;         
            $roi_wallet_balance =  custom_round($diff,3);
            if ($request->working_wallet <= $roi_wallet_balance) {
                $insertId=0;
                ...
                if ($auto_withdrawal_limit != NULL && $auto_withdrawal_limit > 0 && $projectSettings->auto_withdrawal_status == 'on') {
                    if ($request->working_wallet <= $auto_withdrawal_limit && $auto_status == 1) {
                ...
                $amount = $request->input('working_wallet');
                $updateData = array();
