...
class ReportsController extends Controller {
	...
	public function closestatus(Request $request){
		...
		$time =	ProjectSetting::select('closing_time')->first();
		
		$data = Topup::select('entry_time')->where([['id', $Checkexist],['compound_lock_status', 0]])->first();

		$current_time = \Carbon\Carbon::now()->toDateTimeString();

		 $date = date('Y-m-d H:i:s', strtotime(date("Y-m-d H:i:s", strtotime($data)) . " +1 day"));

		 // dd($date);
 
		// $CheckDate = date($data . " +1 day" .$time);	

		// $data = Topup::where([['id', $Checkexist],['compound_lock_status', 0],['entry_time','<', $CheckDate]])->update(['compound_lock_status' => 2]);

			$arrStatus = Response::HTTP_OK;
			$arrCode = Response::$statusTexts[$arrStatus];
			$arrMessage = 'Lock time has been Ended';
			return sendResponse($arrStatus, $arrCode, $arrMessage, $data);
	}

	public function Updatelokingstatus(Request $request){
		...
		if (!empty($Checkexist)) {
			$num = 1;
			$current_time = \Carbon\Carbon::now()->toDateTimeString();
			$data = Topup::where([['id', $Checkexist],['pin', $sr]])->update(['compound_lock_status' => $num,'compound_last_roi_entry_time' =>$current_time]);
			$arrStatus = Response::HTTP_OK;
			$arrCode = Response::$statusTexts[$arrStatus];
			$arrMessage = 'Locked Successful';
			return sendResponse($arrStatus, $arrCode, $arrMessage, $data);
		} else {
				$arrStatus = Response::HTTP_NOT_FOUND;
				$arrCode = Response::$statusTexts[$arrStatus];
	...
	public function Updatelokingstatuscheckbox(Request $request){
			...
			if (!empty($Checkexist)) {
				...
				*/
				$topupReport = Topup::join('tbl_product as tp', 'tp.id', '=', 'tbl_topup.type')/*->leftjoin('tbl_users as tu', 'tu.id', '=', 'tbl_topup.franchise_id')*/
					->select('tbl_topup.srno','tbl_topup.product_name','tbl_topup.pin','tbl_topup.amount','tbl_topup.entry_time','tbl_topup.compound_last_roi_entry_time','tbl_topup.compound_lock_status', 'tp.name',DB::raw('(CASE  WHEN tbl_topup.topupfrom = "Admin Topup" THEN "Admin Topup" ELSE tbl_topup.topupfrom END ) as topupfrom'))
	...
	public function pendingAddDeposit(Request $request) {
		try {
			...
			// ini_set('memory_limit', '-1');
			$arrInput = $request->all();
			$pendingReport = TransactionInvoice::select('srno', 'invoice_id', 'price_in_usd', 'payment_mode', 'address', 'entry_time', 'status_url','product_url',DB::raw('(CASE tbl_transaction_invoices.in_status WHEN 0 THEN "Pending" WHEN 2 THEN "Expired" ELSE "" END) as in_status'))
				->where('id', $id)->whereIn('in_status', [0,2])
	...
	public function CompoundingIncomeReport(Request $request) {
		...
		try {
			...
			if (!empty($userExistId)) {
				...
				$ComoundingDailyBonus = CompoundDailyBouns::join('tbl_product as tp', 'tbl_compound_dailybouns.type', '=', 'tp.id')
								/*->join('tbl_topup as tt', 'tt.pin', '=', 'tbl_compound_dailybouns.pin')*/
								->select('tbl_compound_dailybouns.sr_no','tbl_compound_dailybouns.pin','tbl_compound_dailybouns.amount','tbl_compound_dailybouns.on_amount','tp.name','tbl_compound_dailybouns.entry_time','tbl_compound_dailybouns.status')
								->where('tbl_compound_dailybouns.id', '=', $userExistId);
				...
				if (isset($arrInput['deposit_id'])) {
				    $ComoundingDailyBonus  = $ComoundingDailyBonus->where('tbl_compound_dailybouns.pin', $arrInput['deposit_id']);
				}
		...
		try {
			...
			if ($userexist) {
				...
				if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					...
					$transaction = $transaction->whereBetween(DB::raw("DATE_FORMAT(tbl_all_transaction.transaction_date,'%Y-%m-%d')"), [$arrInput['frm_date'], $arrInput['to_date']]);
				}
	...
	/**
	 * Working Transfer report
	 *
	 * @return void
	 */
	public function workingTransferReport(Request $request) {
		...
		try {
			...
			// ini_set('memory_limit', '-1');
			if ($userexist) {
				$transaction = DexToPurchaseFundTransfer::join('tbl_users as tu', 'tu.id', '=', 'tbl_dex_to_purchase_transfer.from_user_id')->select('tbl_dex_to_purchase_transfer.amount',
				 'tbl_dex_to_purchase_transfer.from_user_id',
					'tbl_dex_to_purchase_transfer.status','tbl_dex_to_purchase_transfer.id',
					'tbl_dex_to_purchase_transfer.from_wallet_type',  
					'tbl_dex_to_purchase_transfer.entry_time', 'tu.user_id')

				->orderBy('tbl_dex_to_purchase_transfer.id', 'DESC')
				->where([['tbl_dex_to_purchase_transfer.from_user_id', $userexist], ['tbl_dex_to_purchase_transfer.from_wallet_type', '1']]);				
				...
				if (!empty($request->input('search')['value']) && isset($request->input('search')['value'])) {
					...
					$transaction = $transaction->where(function ($transaction) use ($fields, $search) {
						foreach ($fields as $field) {
							$transaction->orWhere('tbl_dex_to_purchase_transfer.' . $field, 'LIKE', '%' . $search . '%');
						}
						$transaction->orWhere('tu.user_id', 'LIKE', '%' . $search . '%');
					});
				}
				...
				if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
					$transaction = $transaction->whereBetween(DB::raw("DATE_FORMAT(tbl_dex_to_purchase_transfer.entry_time,'%Y-%m-%d')"), [date('Y-m-d', strtotime($arrInput['frm_date'])), date('Y-m-d', strtotime($arrInput['to_date']))]);
				}
	 ...
	 * @return void
	 */
	public function roiTransferReport(Request $request) {
		...
		try {
			...
			if ($userexist) {
				...
				->orderBy('tbl_dex_to_purchase_transfer.id', 'DESC')
				->where([['tbl_dex_to_purchase_transfer.from_user_id', $userexist], ['tbl_dex_to_purchase_transfer.from_wallet_type', '3']]);				
	...
	/**
	 * Compounding Transfer report
	 *
	 * @return void
	 */
	public function compoundingTransferReport(Request $request) {
		...
		try {
			...
			if ($userexist) {
				...
				->orderBy('tbl_dex_to_purchase_transfer.id', 'DESC')
				->where([['tbl_dex_to_purchase_transfer.from_user_id', $userexist], ['tbl_dex_to_purchase_transfer.from_wallet_type', '2']]);				
	...
	public function DirectTeamReport(Request $request) {
		try {
			$Checkexist = Auth::User()->id;
			...
			if (!empty($Checkexist)) {
				...
				$query = User::select('user_id','fullname','entry_time','ref_user_id','topup_status','status')
									   ->where('ref_user_id', '=', $Checkexist)
 ...
 print_r($query);
	...
	public function RewardIncomeReport(Request $request) {
		try {
			$Checkexist = Auth::User()->id;
			...
			if (!empty($Checkexist)) {
				...
				->join('tbl_reward_setting as trs', 'trs.id', '=', 'tbl_reward_achievers.reward_id')
									   ->select('tbl_reward_achievers.reward_name' ,'tbl_reward_achievers.status','trs.reward_amount','tbl_reward_achievers.entry_time','tu.user_id as user_id')
									   ->where('tbl_reward_achievers.user_id', '=', $Checkexist);
	...
	public function TeamRewardIncomeReport(Request $request) {
		try {
			$Checkexist = Auth::User()->id;
			...
			if (!empty($Checkexist)) {
				...
				->join('tbl_direct_reward_setting as tdrs', 'tdrs.id', '=','tbl_team_direct_reward_achievers.direct_reward_id')
									   ->select('tdrs.required_direct','tdrs.reward_amount','tbl_team_direct_reward_achievers.status','tbl_team_direct_reward_achievers.entry_time','tu.user_id as user_id')
									   ->where('tbl_team_direct_reward_achievers.user_id', '=', $Checkexist);
// dd($query);
				if (isset($arrInput['user_id'])) {
				    $query  = $query->where('tu.user_id', $arrInput['user_id']);
				}
			...
			if (!empty($userId)) {
				...
				for ($i = 0; $i <= ($currmonth-1); $i++) {
					...
					$binaryData = PayoutHistory::selectRaw('ANY_VALUE(created_at) as entry_time,ANY_VALUE(sum(amount)) as amount')->where('tbl_payout_history.user_id', '=', $userId)
						...
						$franchiseData = DB::table('tbl_franchise_income')
						->selectRaw('ANY_VALUE(entry_time) as entry_time,ANY_VALUE(sum(amount)) as amount')->where('tbl_franchise_income.to_user_id', '=', $userId)
						->where(DB::raw("DATE_FORMAT(entry_time,'%Y-%m')"), '=', $Todate)
						->groupBy(DB::raw("DATE_FORMAT(entry_time,'%Y-%m')"))->first();

						$teamRewardData = DB::table('tbl_team_direct_reward_achievers')
           				 ->join('tbl_direct_reward_setting as tdrs', 'tdrs.id', '=','tbl_team_direct_reward_achievers.direct_reward_id')

						->selectRaw('ANY_VALUE(tbl_team_direct_reward_achievers.entry_time) as entry_time,ANY_VALUE(sum(tdrs.reward_amount)) as amount')->where('tbl_team_direct_reward_achievers.user_id', '=', $userId)
						->where(DB::raw("DATE_FORMAT(tbl_team_direct_reward_achievers.entry_time,'%Y-%m')"), '=', $Todate)
						->groupBy(DB::raw("DATE_FORMAT(tbl_team_direct_reward_achievers.entry_time,'%Y-%m')"))->first();


					$rewardData = DB::table('tbl_reward_achievers')
            			->join('tbl_reward_setting as trs', 'trs.id', '=', 'tbl_reward_achievers.reward_id')
						->selectRaw('ANY_VALUE(tbl_reward_achievers.entry_time) as entry_time,ANY_VALUE(sum(trs.reward_amount)) as amount')->where('tbl_reward_achievers.user_id', '=', $userId)
						->where(DB::raw("DATE_FORMAT(tbl_reward_achievers.entry_time,'%Y-%m')"), '=', $Todate)
						->groupBy(DB::raw("DATE_FORMAT(tbl_reward_achievers.entry_time,'%Y-%m')"))->first();

						$compoundingData = DB::table('tbl_compound_dailybouns')            			
						->selectRaw('ANY_VALUE(entry_time) as entry_time,ANY_VALUE(sum(amount)) as amount')->where('tbl_compound_dailybouns.id', '=', $userId)
						->where(DB::raw("DATE_FORMAT(entry_time,'%Y-%m')"), '=', $Todate)
						->groupBy(DB::raw("DATE_FORMAT(entry_time,'%Y-%m')"))->first();
					...
					$arrData['reward'] = round((isset($rewardData->amount) && !empty($rewardData->amount) ? $rewardData->amount : 0),2);

					$arrData['team_reward'] = round((isset($teamRewardData->amount) && !empty($teamRewardData->amount) ? $teamRewardData->amount : 0),2);

					$arrData['franchise'] = round((isset($franchiseData->amount) && !empty($franchiseData->amount) ? $franchiseData->amount : 0),2);

					$arrData['compounding'] = round((isset($compoundingData->amount) && !empty($compoundingData->amount) ? $compoundingData->amount : 0),2);
