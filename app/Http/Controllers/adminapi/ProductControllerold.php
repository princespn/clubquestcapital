...
*/
			...
			{	
				...
				$comp_lock_per = $objProduct->compounding_lock_percentage;
					$total_comp_lock_per = $objProduct->total_compounding_lock_percentage;
				$invoice_id = substr(number_format(time() * rand(), 0, '', ''), 0, '15');
				...
				$arrInsert = [
					...
					'compounding_lock_percentage' => $comp_lock_per,
					'total_compound_amount' =>($amount*$total_comp_lock_per)/100, 
					'total_compounding_lock_percentage' => $total_comp_lock_per,
					'duration' => $objProduct->duration,
					'direct_income' => $directPer,
					'product_name' => $objProduct->package_name,
					/*'payment_type' => $arrInput['payment_type'],*/
					'top_up_by' => $top_up_by,
					'top_up_type' => 2, //admin topup type
					'topupfrom'=>'Admin Topup',
				...
				if (!empty($storeId)) {
				   ...
				   $active_investment =$udash->total_investment; 
					...
					$ref_direct_income = Topup::where('user_status','Active')->where('id',$touser->ref_user_id)->orderBy('srno', 'desc')->select('direct_income')->first();

			if(!empty($ref_direct_income))
            {
					if ($settings->direct_plan == 'on' && $ref_direct_income->direct_income > 0) {
...
/*		$query = supermatching::select('tu.user_id','tu.fullname','tbl_super_matching.entry_time','tbl_super_matching.pin','tbl_super_matching.rank')
	...
	public function getTopups(Request $request) {
		...
		if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
			$qry = $query;
			$qry = $qry->selectRaw('tu.user_id,tbl_topup.topupfrom,tbl_topup.product_name,tu.fullname,tbl_topup.pin as deposite_id,tbl_topup.amount,tbl_topup.roi_stop_status,tp.name as product,tu1.user_id as topup_by,tbl_topup.payment_type,tbl_topup.entry_time,tbl_topup.roi_status,tbl_topup.ip_address');
			$records = $qry->get();
			$res = $records->toArray();
			if (count($res) <= 0) {
				return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
	...
	public function getLoginCountUsersDetails(Request $request) {
		...
		if (isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
			$arrInput['frm_date'] = date('Y-m-d', strtotime($arrInput['frm_date']));
			$arrInput['to_date'] = date('Y-m-d', strtotime($arrInput['to_date']));
			$query = $query->whereBetween(DB::raw("DATE_FORMAT(attempted_at,'%Y-%m-%d')"), [$arrInput['frm_date'], $arrInput['to_date']]);
		}
	...
	public function getdailyreport(Request $request) {
		...
		if(isset($arrInput['frm_date']) && isset($arrInput['to_date'])) {
            $arrInput['frm_date'] = date('Y-m-d',strtotime($arrInput['frm_date']));
            $arrInput['to_date']  = date('Y-m-d',strtotime($arrInput['to_date']));
            $query = $query->whereBetween(DB::raw("DATE_FORMAT(tbl_daily_report.entry_time,'%Y-%m-%d')"),[$arrInput['frm_date'], $arrInput['to_date']]);
        }
