 ...
 $rules = array(
   ...
   // dd($user,222);
	...
	public function updateUser(Request $request) {
		...
		$rules = [
			'id' => 'required',
			'fullname' => 'required|min:5|max:30|regex:/^[A-Za-z0-9 _]*[A-Za-z0-9][A-Za-z0-9 _]*$/',
		...
		$ruleMessages = array(
			...
			'mobile.min'=> 'The mobile field must be at least 6 characters.',
			'fullname.min'=> 'The fullname field must be at least 5 characters.',
		);
		$validator = Validator::make($arrInput, $rules);
		// $id = Auth::User()->id;
		// $arrInput['user_id'] = $id;
  ...
  //           }
		...
		if ($validator->fails()) {
			...
			$oldUserData = User::select('id','user_id','fullname','email','mobile','city','address','country','sol_address','btc_address','ltc_address','trn_address','doge_address','usdt_trc20_address','ethereum','paypal_address','paypal_address','perfect_money_address','ethereum')->where('id', trim($arrInput['id']))->first();
			...
			if (!empty($oldUserData)) {
				...
				$arrInsertLog = [
					...
					'bnb_address' => $oldUserData->bnb_address,
					'btc_address' => $oldUserData->btc_address,
					'trn_address' => $oldUserData->trn_address,
					'sol_address' => $oldUserData->sol_address,
					'doge_address' => $oldUserData->doge_address,
					'usdt_trc20_address' => $oldUserData->usdt_trc20_address,
				...
				if (!empty($saveOldData)) {
					$arrUpdate = [
						...
						//'trx_address' => $arrInput['trx_address'],
					...
					if (!empty($request->Input('trx_address'))) {
						...
						if(empty($addTRXStatus)){
							if(substr($request->Input('trx_address'), 0,1) == 'T' || substr($request->Input('trx_address'), 0,1) == 't' ){
								if(strlen(trim($request->Input('trx_address'))) >= 26 && strlen(trim($request->Input('trx_address'))) <= 42){
					...
					$addData['sol_address'] = trim($request->Input('sol_address'));
					$addBTCStatus = UserWithdrwalSetting::where([['id',$arrInput['id']],['currency',$addData['currency']],['status',1]])->first();
					if(empty($addBTCStatus)){
						if(strlen(trim($request->Input('sol_address'))) >= 26 && strlen(trim($request->Input('sol_address'))) <= 45){

						}else{
							return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Solona address is not valid!', '');
						}
						$updateData = UserWithdrwalSetting::create($addData);
				...
				if (!empty($request->Input('ethereum'))) {
					$addData['currency'] = "Ethereum";
					$addData['ethereum'] = trim($request->Input('ethereum'));
					$addBTCStatus = UserWithdrwalSetting::where([['id',$arrInput['id']],['currency',$addData['currency']],['status',1]])->first();
					if(empty($addBTCStatus)){
						if(strlen(trim($request->Input('ethereum'))) >= 26 && strlen(trim($request->Input('ethereum'))) <= 45){

						}else{
							return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Ethereum address is not valid!', '');
						}
						$updateData = UserWithdrwalSetting::create($addData);
...
*/
			   ...
			   if($checkUserExist->type != 'sub-admin' || $checkUserExist->type != 'sub-admin' )
			   	{
					...
					// if(!empty($bvcount))
					{
						...
						$arrObject['is_franchise'] = $checkUserExist->is_franchise;
	...
	public function getUserUpdatedLog(Request $request) {
		...
		$query = UsersChangeData::join('tbl_users as tbu', 'tbu.id', '=', 'tbl_users_change_data.updated_by')
			...
			->selectRaw('tbu1.id,tbu1.user_id,tbu2.user_id as sponser_id,tbu2.fullname as sponser,tbu1.mobile as new_mobile,tbu1.fullname as new_name,tbl_users_change_data.fullname,tbl_users_change_data.mobile,tbl_users_change_data.email,cn.country,tbl_users_change_data.btc_address,tbl_users_change_data.ip,tbu.user_id as updated_by,tbl_users_change_data.created_at,tbl_users_change_data.entry_time,
				tbl_users_change_data.trn_address,
				tbl_users_change_data.sol_address,
				tbl_users_change_data.btc_address,
				tbl_users_change_data.ethereum,
				tbl_users_change_data.usdt_trc20_address,
				tbl_users_change_data.ltc_address,
				tbl_users_change_data.doge_address
				')
			->orderBy('tbl_users_change_data.type', 'desc');
		if (isset($arrInput['id'])) {
  ...
  //           }
	...
	public function getUserProfileDetails(Request $request) {
		...
		$getUserLogs = DB::table('tbl_users_change_data as tbucd')
			->selectRaw('tbucd.id,tbucd.fullname,tbucd.mobile,tbucd.btc_address,tbucd.sol_address,tbucd.ltc_address,tbucd.ethereum,tbucd.ref_user_id as sponser_id,tbucd.ip,tu1.fullname as updated_by,tbucd.created_at,tbucd.entry_time,cn.country,tu2.user_id as sponser')
	...
	public function userProfile(Request $request) {
		...
		$getuser = User::select('tbl_users.*', 'cn.iso_code', 'tu.user_id as ref_user_id', 'tu.fullname as sponser_id', DB::raw('(CASE tbl_users.position WHEN 1 THEN "Left" WHEN 2 THEN "Right" WHEN 3 THEN "Right" ELSE "" END) as position'), 'cn.country', DB::raw('DATE_FORMAT(tbl_users.entry_time,"%Y/%m/%d %H:%i:%s") as entry_time'))
			->join('tbl_users as tu', 'tu.id', '=', 'tbl_users.ref_user_id')
			->leftjoin('tbl_country_new as cn', 'cn.iso_code', '=', 'tbl_users.country')
	...
	public function addPower(Request $request) {
		...
		$rules = array('id' => 'required', 'position' => 'required', 'power_bv' => 'required|integer');
		$validator = Validator::make($arrInput, $rules);
  ...
  public function saveBusinessSetting(Request $request){
	...
	public function addBussinessbv(Request $request) {
		...
		$rules = array('id' => 'required', 'power_bv' => 'required', 'position' => 'required', 'type' => 'required');
		$validator = Validator::make($arrInput, $rules);
		$id = Auth::User()->id;
		$arrInput['user_id'] = $id;
			$arrRules            = ['otp' => 'required|min:6|max:10'];
            // $verify_otp = verify_Otp($request);
			// dd($verify_otp);
            $verify_otp = Otp::select('otp_id','otp_status','otp')
            ->where('id', Auth::user()->id)
            ->where('otp', md5($request->otp))
	...
	public function buinessUplineReport(Request $request) {
		...
		if (!empty($userExist)) {
			...
			if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
				$qry = $query;
				$qry = $qry->select('tbl_add_remove_business_upline.power_bv','tbl_add_remove_business_upline.position',
			'tbl_add_remove_business_upline.before_lbv','tbl_add_remove_business_upline.before_rbv',
			'tbl_add_remove_business_upline.after_lbv','tbl_add_remove_business_upline.after_rbv', 
			'tbl_add_remove_business_upline.before_curr_lbv','tbl_add_remove_business_upline.before_curr_rbv', 
			'tbl_add_remove_business_upline.after_curr_lbv','tbl_add_remove_business_upline.after_curr_rbv', 'tbl_add_remove_business_upline.remark','tbl_add_remove_business_upline.type',
	...
	public function uplineBusinessReport(Request $request) {
		...
		if (!empty($userExist)) {
			...
			if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
				$qry = $query;
				$qry = $qry->select('tbl_add_remove_business_upline.*', 'tu.user_id as user', 'tu.fullname', DB::raw('DATE_FORMAT(tbl_add_remove_business_upline.entry_time,"%Y/%m/%d") as entry_time'));
				// $qry = $qry->select('tu.user_id as user', 'tu.fullname', 'tbl_add_remove_business_upline.power_bv','tbl_add_remove_business_upline.position' , 'tbl_add_remove_business_upline.before_lbv' , 'tbl_add_remove_business_upline.before_rbv' , 'tbl_add_remove_business_upline.after_lbv' , 'tbl_add_remove_business_upline.after_rbv' , 'tbl_add_remove_business_upline.before_curr_lbv' , 'tbl_add_remove_business_upline.before_curr_rbv' , 'tbl_add_remove_business_upline.after_curr_lbv' , 'tbl_add_remove_business_upline.after_curr_rbv' , 'tbl_add_remove_business_upline.remark' , 'tbl_add_remove_business_upline.cron_status' ,  DB::raw('DATE_FORMAT(tbl_add_remove_business_upline.entry_time,"%Y/%m/%d") as entry_time'));
				$records = $qry->get();
				$res = $records->toArray();
				if (count($res) <= 0) {
					return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
	...
	public function updateFranchiseUserStatus(Request $request)
	{

		$arrInput = $request->all();
		$rules = array(
			'user_id' => 'required',
			'franchise_for' => 'required',
		);
		$validator = Validator::make($arrInput, $rules);
		if ($validator->fails()) {
			$message = messageCreator($validator->errors());
		...
		$userExist = User::where([['user_id', '=', $arrInput['user_id']],['status','Active']])->first();

		if (!empty($userExist)) {
			if($arrInput['franchise_for'] == '1'){
				$updateData['is_franchise'] = 1;
				$updateData['income_per'] = 2;
				$msg = "Successfully registered as franchise user";
			}else if($arrInput['franchise_for'] == '2'){
				$updateData['is_franchise'] = 0;				
				$updateData['income_per'] = 0;				
				$msg = "Successfully removed as franchise user";
			}
			// dd($updateData);
			$updateOtpSta = User::where('id', $userExist->id)->update($updateData);
			return sendresponse($this->statuscode[200]['code'], $this->statuscode[200]['status'], $msg, '');
		} else {
			return sendresponse($this->statuscode[401]['code'], $this->statuscode[401]['status'], 'User ID is not valid.', '');
		}
	}
