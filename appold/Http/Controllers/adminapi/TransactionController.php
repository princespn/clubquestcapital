...
class TransactionController extends Controller
{
   ...
   public function getPerfectmoneyReport(Request $request) {
    ...
    public function getConfirmAddrTrans(Request $request) {
        ...
        if (isset($arrInput['action']) && $arrInput['action'] == 'export') {
            $qry = $query;
            $qry = $qry->selectRaw('tu.user_id,tu.fullname,tbl_transaction_invoices.address,tbl_transaction_invoices.status_url as checkout_url,tbl_transaction_invoices.hash_unit as amount,tbl_transaction_invoices.currency_price,tbl_transaction_invoices.payment_mode,(CASE tbl_transaction_invoices.in_status WHEN 0 THEN "Pending" WHEN 2 THEN "Expired" WHEN 1 THEN "Confirm" ELSE "" END) as status,tbl_transaction_invoices.entry_time');
            $records = $qry->get();
            $res = $records->toArray();
            if (count($res) <= 0) {
                return sendresponse($this->statuscode[404]['code'], $this->statuscode[404]['status'], 'Data not found', array());
