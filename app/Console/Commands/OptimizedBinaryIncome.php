  ...
  public function handle()
    {
        ...
        /* ->join('tbl_topup as tt', 'tt.id', '=', 'tbl_users.id')*/
         ->where([['tbl_users.curr_l_bv','>',0],['tbl_users.curr_r_bv','>',0],['tbl_users.status','=',"Active"],['tbl_users.type','!=',"Admin"]
            /*,['tbl_users.binary_qualified_status','=',1],['tbl_users.topup_status','=',"1"]*/])
         //->limit(1)
         ->get();
            ...
            foreach ($getUserDetails as $key => $value) 
            {
                ...
                $type = $value->type;
                    ...
                    {*/
                        ...
                        $binary_per = (!empty($topup_amt))?$topup_amt->binary_percentage:10;  
                        $capping = (!empty($topup_amt))?$topup_amt->binary_capping:0;
                        
                        $amount = ($match_bv * $binary_per)/100;
                        $capping_amount  = 0;
                        $remark = "Binary Income Given";
                        $status = "Paid";

                        $ref_count = User::where('id',$id)->where('type','')->select('topup_status')->first();
                        ...
                        {  
                           ...
                           $amount      =  $capping;
                           $status = "Paid";
                        }elseif($ref_count->topup_status === "0")
                        {
                           /*$capping_amount = $amount;*/
                           $laps_amount =   $amount;
                           $amount      =  0;
