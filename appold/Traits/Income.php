...
trait Income {
  ...
  function payold_binary($user_id, $amount) {
    	try{
            ...
            $setting = ProjectSettings::select('reward_start_time','reward_end_time')->where(['status' => 1])->first();
            if(strtotime($now) >= strtotime($setting->reward_start_time) && strtotime($now) <= strtotime($setting->reward_end_time)){
                $updateLCountArr['reward_lbv'] = DB::raw('reward_lbv + ' . $amount . '');
                $updateLCountArr['reward_curr_lbv'] = DB::raw('reward_curr_lbv + ' . $amount . '');
                
                $updateRCountArr['reward_rbv'] = DB::raw('reward_rbv + ' . $amount . '');
                $updateRCountArr['reward_curr_rbv'] = DB::raw('reward_curr_rbv+ ' . $amount . '');
            }*/
        ...
        try {
            ...
            */
            ...
            while ($count1 <= count($array1)) {
                //dd($array1);
                $key1 = $count1 - 1;
                User::whereIn('id', $array1[$key1])/*->where('topup_status', "1")*/
            ...
            while ($count2 <= count($array2)) {
                $key2 = $count2 - 1;
                User::whereIn('id', $array2[$key2])/*->where('topup_status', "1")*/
    ...
    function pay_binary_new($user_id, $amount) {
    	try{
